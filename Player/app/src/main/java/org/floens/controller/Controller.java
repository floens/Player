...
public abstract class Controller {
    ...
    public BottomBarItem bottomBarItem = new BottomBarItem();
    ...
    private boolean presentingAnimating = false;
    ...
    public void onShow() {
        shown = true;
        if (LOG_STATES) {
            Log.i(TAG, getClass().getSimpleName() + " onShow");
        }
    ...
    public void onHide() {
        shown = false;
        if (LOG_STATES) {
            Log.i(TAG, getClass().getSimpleName() + " onHide");
        }
    ...
    public void onDestroy() {
        ...
        if (AndroidUtils.removeFromParentView(view)) {
            if (LOG_STATES) {
                Log.i(TAG, getClass().getSimpleName() + " view removed onDestroy");
            }
        }
    }
    ...
    public void attachToParentView(ViewGroup parentView) {
        if (view.getParent() != null) {
            if (LOG_STATES) {
                Log.i(TAG, getClass().getSimpleName() + " view removed");
            }
        ...
        if (parentView != null) {
            if (LOG_STATES) {
                Log.i(TAG, getClass().getSimpleName() + " view attached");
            }
    ...
    public void presentController(Controller controller, boolean animated) {
        presentController(controller, animated ? new FadeInTransition() : null);
    }

    public void presentController(Controller controller, ControllerTransition transition) {
        if (presentingAnimating) {
            return;
        }
    ...
    public void stopPresenting(boolean animated) {
        stopPresenting(animated ? new FadeOutTransition() : null);
    }

    public void stopPresenting(ControllerTransition transition) {
        if (presentingAnimating) {
            return;
        }
